...
# Function defined to count the characters in a book
    # print(text) # cheks if uppercase is still applied (comment out as soon as debugged)
    text = text.lower() # converts the book to lower case to be read by the function
    # print(text) # cheks if the lower case is applied (comment out as soon as debugged)
    num_chars_dict = {} # necessary for the rest of the function, stores the char and its count 
...
# Function to sort the characters and print the layout as requested
    ...
    sorted_chars = [{"char":char, "num":count} 
                    for char, count in char_counts.items()] # using a 'list comprehension' instead of 'loop and append'
